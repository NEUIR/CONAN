...
class DRTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        if self.args.distillation:
            ...
            # outputs_reverse = model(query=passage, passage=query)

        logger.info("   normal loss")
        # logger.info("   nl2code loss")
        # logger.info("   code2nl loss")
        return (outputs.loss, outputs) if return_outputs else outputs.loss
        # return (outputs.loss, outputs) if return_outputs else outputs.loss + outputs_reverse.loss
