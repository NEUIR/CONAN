...
class DRModel(nn.Module):
    ...
    def forward(
        ...
        if self.train_args.distillation:
            ...
            # import torch.nn.functional as F
            # q_norm = F.normalize(q_reps, p=2, dim=-1)  # 对查询向量进行 L2 归一化 p：默认为2，表示2范数；同理，p=1表示1范数
            # p_norm = F.normalize(p_reps, p=2, dim=-1)  # 对待匹配项向量进行 L2 归一化
            # cos_sim = torch.matmul(q_norm, p_norm.transpose(0, 1))  # 计算余弦相似度
            # temperature = 0.02
            # scores = cos_sim / temperature  # 将余弦相似度按照温度值进行缩放
    ...
    def encode(self, items, model, head, is_q=False):
        ...
        if "T5" in type(model).__name__ and not self.model_args.encoder_only:
            ...
            items_out = model(**items, decoder_input_ids=decoder_input_ids, return_dict=True, output_hidden_states=True)
