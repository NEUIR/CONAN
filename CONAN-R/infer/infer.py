    ...
    for batch in dataloader:
        with torch.no_grad():
            ...
            outputs = model(
                ...
                output_hidden_states=True,  # Save all hidden states
            )

            # Get the last layer hidden states from the encoder
            hidden = outputs.hidden_states[-1]  # In RoBERTa, use hidden_states instead of decoder_hidden_states
            vec = hidden[:, 0, :]
...
def main():
    ...
    model = RobertaModel.from_pretrained(args.pretrained_dir)
