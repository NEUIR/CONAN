...
class FiDT5(transformers.T5ForConditionalGeneration):
    ...
    #  kwargs 参数，它代表了一个字典类型的参数集合，允许在调用该方法时传入任意数量的关键字参数。
    def forward_(self, **kwargs):
        if 'input_ids' in kwargs:
            # view() 是 PyTorch 张量（tensor）的一个方法，它用于改变张量的形状，即改变张量的维度。
    ...
    def generate(self, input_ids, attention_mask, max_length,use_cache=True,early_stopping=False,num_beams=10):
        self.encoder.n_passages = input_ids.size(1)
